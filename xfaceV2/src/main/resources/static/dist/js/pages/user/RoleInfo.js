$(document).ready( function () {
	...
	dataTable.DataTable( {
        ...
        "columns": [
            ...
            { "data": "htmlPageInfo.pageDesc","name" : "htmlPageInfo.pageDesc"}
        ]
	...
	function showDialog(roleInfo) { 
    	...
    	if (roleInfo === null) {
    		...
    		$("#txtRoleName").val("");    		
    		$("#cmbDefaultPage").selectpicker("val", defaultPageCode);    
    		$("#txtRoleCode").focus();
    		...
    		$("#cmbDefaultPage").selectpicker("val", roleInfo.htmlPageInfo.pageCode);
    		$("#txtRoleName").focus();
	...
	function bindPermission(permissionList){
...
//		}		
	...
	/////////////////////	
	//get html page info list form backend and bind to screen
	function getHtmlPageInfo() {
		globalWriteConsoleLog("in getHtmlPageInfo");
        $.ajax({
            type: "GET",
            url: "/xFace/rest/cfg/getAllHtmlPageInfo",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function(htmlPageInfoList) {
            	globalIsSessionExpire(htmlPageInfoList);
            	bindHtmlPageInfoList(htmlPageInfoList);            	
            },
            error: function(error){
            	globalClearLoader();
        ...
        globalWriteConsoleLog("out getHtmlPageInfo");
    }
	
	//bind page info to screen
	function bindHtmlPageInfoList(htmlPageInfoList){
		var cmbDefaultPage = $("#cmbDefaultPage");
		var options = [];
		$.each(htmlPageInfoList, function(i, item){		
			if (defaultPageCode===null){
				defaultPageCode = item.pageCode; 
			}
			options.push("<option value='"+item.pageCode+"'>"+item.pageDesc+"</option>");			
		});
		//show dialog
